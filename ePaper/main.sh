...
try:
    ...
    epd.Clear(0xFF)
    font24 = ImageFont.truetype('/usr/share/fonts/truetype/wqy/wqy-microhei.ttc', 45)
    ...
    while (True):
        ...
        time_draw.text((15, 54), f"{datetime.datetime.now():%d, %B %Y}", font = font20, fill = 0)

        time_draw.text((25, 95), "IP: " + get_ip_address(), font = font17, fill = 0)
